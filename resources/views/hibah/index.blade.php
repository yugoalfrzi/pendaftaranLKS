...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">
        ...
        <span class="badge bg-secondary ms-2">{{ $selectedYear ?? now()->year }}</span>
    </h1>
    @auth
...
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('hibah.keuangan', $selectedYear ?? now()->year) }}" class="row g-3">
            <div class="col-md-4">
            ...
            <div class="col-md-4 d-flex align-items-end gap-2">
                ...
                <a href="{{ route('hibah.keuangan', $selectedYear ?? now()->year) }}" class="btn btn-outline-secondary w-100">
                    Reset
                </a>
            </div>
        </form>
...
<div class="card">
    <div class="card-header bg-white">
        <h5 class="card-title mb-0">
            <i class="bi bi-table"></i> Daftar Data Hibah Tahun {{ $selectedYear ?? now()->year }}
        </h5>
    </div>
    <div class="card-body">
        @if($items->count() > 0)
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <strong>{{ $row->nama_lks }}</strong><br>
                            <small class="text-muted">Tahun: {{ $row->tahun }}</small>
                        </td>
                        <td>
                            @if($row->proposal_path)
                                <a href="{{ Storage::disk('public')->url($row->proposal_path) }}" target="_blank" class="btn btn-outline-primary btn-sm">
                        ...
                        <td>
                            @if($row->lpj_path)
                                <a href="{{ Storage::disk('public')->url($row->lpj_path) }}" target="_blank" class="btn btn-outline-success btn-sm">
                                    <i class="bi bi-eye"></i> Lihat
                                </a>
                            @else
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('hibah.show', $row) }}" class="btn btn-outline-info" title="Detail">
                                    <i class="bi bi-eye"></i>
                                </a>
                                @auth
                                    @if(Auth::user()->hasRole(['super_admin', 'admin']))
                                    <a href="{{ route('hibah.edit', $row) }}" class="btn btn-outline-warning" title="Edit">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <a href="{{ route('hibah.documents', $row->id) }}" class="btn btn-outline-secondary" title="Dokumen">
                            ...
                            @auth
                                @if(Auth::user()->hasRole(['super_admin', 'admin']))
                                <form action="{{ route('hibah.destroy', $row) }}" method="POST" class="d-inline mt-1"
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted mt-2">Belum ada data hibah untuk tahun {{ $selectedYear ?? now()->year }}</p>
            @auth
                @if(Auth::user()->hasRole(['super_admin', 'admin']))
                <a href="{{ route('hibah.create') }}" class="btn btn-primary">
                    <i class="bi bi-plus-circle"></i> Tambah Data Hibah
