...
class KewenanganKemensosController extends Controller
{
    ...
    {
        return view('kewenangan.kemensos.create');
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'tanda_pendaftaran_pusat_nama_provinsi' => 'required|string|max:255',
            'tanda_pendaftaran_pusat_nomor' => 'required|string|max:255',
            'tanda_pendaftaran_pusat_tanggal' => 'required|date',
            'tanda_pendaftaran_cabang_nama_provinsi' => 'required|string|max:255',
    ...
    public function update(Request $request, KewenanganKemensos $kewenangan)
    {
        $validated = $request->validate([
            'Nama_Lembaga_Yayasan' => 'required|string',
            'status' => 'required|in:pusat,cabang',
    ...
    public function destroy(KewenanganKemensos $kewenangan)
    {
        $kewenangan->delete();

        return redirect()->route('kewenangan-kemensos.index')
                         ->with('success', 'Data Kewenangan Kemensos berhasil dihapus.');
    }
    ...
    private function getStatistics($filterKabkota = null)
    {
        ...
        $statsByKabupaten = KewenanganKemensos::select(
            'kabupaten_kota',
            DB::raw('SUM(jumlah_dalam_panti) as total_dalam_panti'),
            DB::raw('SUM(jumlah_luar_panti) as total_luar_panti'),
            DB::raw('SUM(jumlah_seluruh_binaan) as total_seluruh_binaan'),
