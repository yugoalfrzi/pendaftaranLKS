...
class SuperAdminController extends Controller
{
    ...
    {
        // Hanya tampilkan LKS yang sudah ada surat rekomendasi (status Diterima dari admin)
        $query = lks::with('user')
        ...
        $stats = [
            'total' => lks::where('status_permohonan', 'Diterima')->whereNotNull('surat_rekomendasi_path')->count(),
            'menunggu' => lks::where('status_permohonan', 'Diterima')->whereNotNull('surat_rekomendasi_path')->whereNull('sertifikat_path')->count(),
            ...
            'with_sertifikat' => lks::where('status_permohonan', 'Diterima')->whereNotNull('surat_rekomendasi_path')->whereNotNull('sertifikat_path')->count(),
        ];
    ...
    public function processVerification(Request $request, $id)
    {
        ...
        // Validasi bahwa LKS sudah memiliki surat rekomendasi
        if (!$lks->surat_rekomendasi_path || $lks->status_permohonan != 'Diterima') {
            return redirect()->route('superadmin.index')
                ->with('error', 'LKS ini belum memiliki surat rekomendasi dari admin.');
        }
